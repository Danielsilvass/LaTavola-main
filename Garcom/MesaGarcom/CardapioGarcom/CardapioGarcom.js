...
const elInfoCG = document.querySelector('.usuario-info, .user-info');
if (elInfoCG) {
  ...
  elInfoCG.appendChild(btnLogout);
}
...
async function carregarCardapio() {
  ...
  // Carregar categorias dinâmicas do Firestore
  try {
    const configSnap = await getDocs(collection(db, "configuracoes"));
    let categorias = [];
    configSnap.forEach((d) => {
      if (d.id === "geral" && d.data().categorias) {
        categorias = d.data().categorias;
      }
    });

    const containerFiltros = document.getElementById("botoesFiltro");
    if (containerFiltros && categorias.length > 0) {
      containerFiltros.innerHTML = `<button class="ativo" data-categoria="Todos">Todos</button>`;
      categorias.forEach((cat) => {
        containerFiltros.innerHTML += `<button data-categoria="${cat}">${cat}</button>`;
      });
      // Reativar eventos nos novos botões
      document.querySelectorAll('#botoesFiltro button').forEach((btn) => {
        btn.addEventListener('click', (e) => {
          document.querySelectorAll('#botoesFiltro button').forEach((b) => b.classList.remove('ativo'));
          e.target.classList.add('ativo');
    ...
    console.warn("Não foi possível carregar categorias dinâmicas:", e);
  }
