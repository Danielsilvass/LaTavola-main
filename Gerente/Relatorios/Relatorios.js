...
const elInfoRel = document.querySelector('.usuario-info, .user-info');
if (elInfoRel) {
  ...
  elInfoRel.appendChild(btnLogout);
}
...
onSnapshot(produtosRef, (snapshot) => {
  snapshot.forEach((doc) => {
    ...
    if (p.ativo && !dictProdutos[p.nome]) {
      ...
      let custoUnidade = Number(p.custo) || 0;
      let lucroUnidade = preco - custoUnidade;
      let margem = preco > 0 ? (lucroUnidade / preco) * 100 : 0;
