...
const elInfoEC = document.querySelector('.usuario-info, .user-info');
if (elInfoEC) {
  ...
  elInfoEC.appendChild(btnLogout);
}
...
onSnapshot(produtosRef, (snapshot) => {
  snapshot.forEach((doc) => {
    ...
    if (p.ativo && !dictProdutos[p.nome]) {
      ...
      const custoReais = Number(p.custo) || 0;
      const lucroReais = preco - custoReais;
      const realMargin = preco > 0 ? (lucroReais / preco) * 100 : 0;
      dictProdutos[p.nome] = {
        nome: p.nome,
